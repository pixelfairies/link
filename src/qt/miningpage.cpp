...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList("Miner failed to start. Make sure you have the minerd executable and libraries in the same directory as Pixels-qt.", ERROR, NULL);
    }
}
...
{
    if (getMiningType() == ClientModel::SoloMining)
        reportToList("Solo minting stopped.", ERROR, NULL);
    else
...
void MiningPage::minerStarted()
{
    if (!minerActive)
        if (getMiningType() == ClientModel::SoloMining)
            reportToList("Solo minting started.", ERROR, NULL);
        else
...
{
    ui->startButton->setText(minerActive ? "Stop Minting" : "Start Minting");
